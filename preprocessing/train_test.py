...
drop_var = ["('cliente_numero_siniestros', 'median')", "('cliente_carga_siniestral', 'median')",
            "('cliente_numero_siniestros_auto', 'median')"]

# cluster cp
cluster_cp = cluster_cp.rename(columns={"('cliente_cp', '')": 'oferta_tomador_cp', 'labels': 'cp_risk'})
cluster_cp['cp_control'] = cluster_cp['oferta_tomador_cp']
cust_risk_relabel = cluster_cp.groupby(['cp_risk']).agg(
    {"('cliente_numero_siniestros', 'median')": 'mean', "('cliente_carga_siniestral', 'median')": 'mean',
     "('cliente_numero_siniestros_auto', 'median')": 'mean'}).reset_index(
    drop=False)

weighted = 2/3
cust_risk_relabel['weight_risk'] = (1-weighted/2) * cust_risk_relabel["('cliente_numero_siniestros', 'median')"] / \
                                   cust_risk_relabel["('cliente_numero_siniestros', 'median')"].max() + (1-weighted/2) * \
                                   cust_risk_relabel["('cliente_carga_siniestral', 'median')"] / cust_risk_relabel[
                                       "('cliente_carga_siniestral', 'median')"].max() + weighted * cust_risk_relabel[
                                       "('cliente_numero_siniestros_auto', 'median')"] / cust_risk_relabel[
                                       "('cliente_numero_siniestros_auto', 'median')"].max()

cust_risk_relabel = cust_risk_relabel.sort_values(by=['weight_risk'], ascending=[True]).reset_index(drop=True)
...
int_risk_relabel = cluster_intm.groupby(['intm_risk']).agg({'mediador_riesgo': 'mean'}).reset_index(
    drop=False).sort_values(by=['mediador_riesgo'], ascending=[True]).reset_index(drop=True)
...
offer_processed = pd.merge(offer_processed, cluster_cia[['oferta_sim_cia_actual', 'cia_risk_relabel']],
                           how='left', on='oferta_sim_cia_actual')
...
offer_processed = pd.merge(offer_processed, cluster_cp[['oferta_tomador_cp', 'cp_risk_relabel', 'cp_control']], on='oferta_tomador_cp', how='left')
...
offer_processed = pd.merge(offer_processed, cluster_intm[['oferta_cod_intermediario', 'intm_risk_relabel']],
                           how='left', on='oferta_cod_intermediario')
...
offer_processed = pd.merge(offer_processed, cluster_customer[['cliente_edad', 'antiguedad_permiso_range',
                                                                ...
                                                                how='left', on=['cliente_edad', 'antiguedad_permiso_range',
                                                                               'edad_segundo_conductor_riesgo',
                                                                               'oferta_tomador_sexo'
                                                                               ])
...
print(len(offer_processed.index))
# MATCH OBJECT
offer_processed = pd.merge(offer_processed, cluster_veh, how='left', on=['oferta_veh_valor', 'oferta_veh_categoria',
...
# offer_processed = offer_processed[offer_processed['oferta_bonus_simulacion_perc'] <= 0.55]
print(len(offer_processed.index))
...
# PLOTS
plot.hist(offer_processed['oferta_bonus_simulacion_perc'] - offer_processed['oferta_nivel_sinco_perc'])
plot.hist(offer_processed['oferta_nivel_sinco_perc'])


offer_processed2 = offer_processed[offer_processed['oferta_bonus_simulacion_perc'] > 10]
...
offer_processed.loc[(offer_processed['oferta_bonus_simulacion_perc'] - offer_processed['oferta_nivel_sinco_perc'] >= 5)
                    &(offer_processed['oferta_nivel_sinco_perc'] <= 10),
                    'target'] = 1
'''
offer_processed.loc[(offer_processed['oferta_nivel_sinco_perc'] < 0)&(offer_processed['oferta_bonus_simulacion_perc'] >= 10), 'target'] = 1
'''
#####################################################################################################################
...
offer_processed = offer_processed[
    ...
    'edad_segundo_conductor_riesgo',  'oferta_veh_valor_unitary',
     ...
     'customer_risk_relabel', 'veh_risk_relabel', 'target']]
'''
'''
offer_processed = offer_processed[
    ['oferta_id', 'oferta_tomador_sexo', 'oferta_tom_cond', 'oferta_propietario_tom', 'oferta_propietario_cond',
     'oferta_veh_categoria', 'antiguedad_permiso', 'edad_segundo_conductor_riesgo',
     'car_ranking', 'd_tipo_furgoneta', 'oferta_bonus_simulacion', 'oferta_veh_valor_unitary',
     'cliente_edad_30_65', 'cliente_edad_65', 'antiguedad_vehiculo',
     'cia_risk_relabel', 'cp_risk_relabel', 'intm_risk_relabel',
     'customer_risk_relabel', 'veh_risk_relabel'
        , 'target']]
'''

# del offer_processed['cp_control']
